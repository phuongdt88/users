...
var compile = function (fileName, assets, S3, options, method, type, timestamp, callback) {
  ...
  return function (err, results) {
    ...
    case 'minify':
      ...
      async.mapSeries(results, function(key, iter) {
        var minify = new cleanCSS().minify(key);
        var assetPath = assets[results.indexOf(key)];
        var assetBasePath = path.dirname(assetPath);
        var fileBasePath = path.dirname(path.join(options.publicDir, fileName));

        // Process images
        minify = minify.replace(/(?:background\-image|background|content|border\-image|cursor|src)\:[^;\n]*\)/g, function (rootMatch) {
          ...
          return rootMatch.replace(/url\((?!data:)['"]?([^\)'"]+)['"]?\)/g, function (match, url) {
            ...
            if (options.production) {
              ...
              timestamp = fs.statSync(fileBasePath).mtime.getTime();
              var imageResource = S3.headFile(relativePath.substr(options.publicDir.length + 1).replace(/\\/g, "/"), checkImageIfModified(relativePath, relativePath.substr(options.publicDir.length + 1).replace(/\\/g, "/"), S3, options, timestamp, mimeType, null, null));
              return 'url(' + path.relative(fileBasePath, relativePath).replace(/\\/g, "/") +'?cache='+ timestamp + ')';
            } else {
              return 'url(' + url + '?cache='+ timestamp + ')';
            }
          });
        });
...
//              }
//              var mimeType = mime.lookup(relativePath);
//              timestamp = fs.statSync(relativePath).mtime.getTime();
//              var fontResource = S3.headFile(relativePath.substr(options.publicDir.length + 1).replace(/\\/g, "/"), checkImageIfModified(relativePath, relativePath.substr(options.publicDir.length + 1).replace(/\\/g, "/"), S3, options, timestamp, mimeType, null, null));
//              return 'url(' + path.relative(fileBasePath, relativePath).replace(/\\/g, "/") +'?cache='+ timestamp + ')';
//            } else {
//              return 'url(' + url +'?cache='+ timestamp + ')';
//            }
      ...
      optipng.on('exit', function (code) {
        ...
        fs.readFile(img, function (err, data) {
          zlib.gzip(data, function (err, buffer) {
            S3.putBuffer(buffer, fileName, headers, function (err, response) {
              if (err) {console.log(err); return finishUpload();}
              if (response.statusCode !== 200) {
                ...
                logger({
                  task: 'express-cdn',
                  message: 'successfully uploaded optipng image "' + fileName + '" to S3'
                });
          ...
          zlib.gzip(data, function (err, buffer) {
            S3.putBuffer(buffer, fileName, headers, function (err, response) {
              ...
              if (response.statusCode !== 200) {
                ...
                logger({
                  task: 'express-cdn',
                  message: 'successfully uploaded jpegtran image "' + fileName + '" to S3'
                });
                // Hack to preserve original timestamp for view helper
...
var checkArrayIfModified = function (assets, fileName, S3, options, timestamp, type, callback) {
  ...
  return function (err, response) {
    ...
    if (response.statusCode === 200 && timestamp <= Date.parse(response.headers['last-modified'])) {
      ...
      console.log('last modified'+ Date.parse(response.headers['last-modified']));
      console.log('STT code: ' + response.statusCode);
      logger({
      ...
      case 'application/javascript':
      case 'text/javascript':
        async.mapLimit(assets,5, readUtf8, compile(fileName, assets, S3, options, 'uglify', type, null, finishUpload));
        return;
      case 'text/css':
        async.mapLimit(assets,5, readUtf8, compile(fileName, assets, S3, options, 'minify', type, null, finishUpload));
        return;
      default:
...
var checkImageIfModified = function (assets, fileName, S3, options, timestamp, callback) {
  var finishUpload = function () {
    return callback;
  };
  return function (err, response) {
...
var processAssets = function (options, results, done) { // Create knox instance
  ...
  async.mapLimit(results,10, function (result, iter) {
    var assets = result,
      type = '',
      fileName = '',
      position, timestamp = 0;
